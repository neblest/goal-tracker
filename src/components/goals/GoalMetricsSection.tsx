        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="space-y-2">
              <Label htmlFor={`${baseId}-name`} className="text-[#4A3F35]">Nazwa</Label>
              <Input
                id={`${baseId}-name`}
              ...
              />
              {errors.name ? (
                <p id={`${baseId}-name-error`} className="text-sm text-[#C17A6F]">{errors.name}</p>
              ) : null}
            </div>

            <div className="space-y-2">
              <Label htmlFor={`${baseId}-target`} className="text-[#4A3F35]">Wartość docelowa</Label>
              <Input
                id={`${baseId}-target`}
              ...
              />
              {errors.target_value ? (
                <p id={`${baseId}-target-error`} className="text-sm text-[#C17A6F]">{errors.target_value}</p>
              ) : null}
            </div>

            <div className="space-y-2">
              <Label htmlFor={`${baseId}-deadline`} className="text-[#4A3F35]">Deadline</Label>
              <Input
                id={`${baseId}-deadline`}
                type="date"
              ...
              />
              {errors.deadline ? (
                <p id={`${baseId}-deadline-error`} className="text-sm text-[#C17A6F]">{errors.deadline}</p>
              ) : null}
            </div>
          </div>
          ...
          {successMessage ? (
            <p className="text-sm text-[#9CAA7F]">{successMessage}</p>
          ) : null}
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="submit" disabled={submitting || !hasChanges} className="bg-[#D4A574] hover:bg-[#C9965E] text-white">
            ...
            <div className="flex items-center gap-4">
              ...
              {!isLocked ? (
                <>
                  ...
                  <Button onClick={() => setIsAbandonModalOpen(true)} variant="outline" size="sm" className="text-red-600 hover:text-red-700">
                    Porzuć
                  </Button>
                </>
              ) : null}
    ...
    <Dialog open={isAbandonModalOpen} onOpenChange={setIsAbandonModalOpen}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Czy na pewno chcesz porzucić cel? Wybierz powód:
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={handleAbandon} disabled={submitting || !selectedReason} className="bg-red-600 hover:bg-red-700 text-white">
